...
class Requests():
    ...
    def home(self, expected):
        req_label = 'uidashboard' # sys._getframe().f_code.co_name + postfix(expected)
        start_time = time.time()
        with self.client.get('/index.html', name = req_label) as response:
            to_log = {'microservice':'uidashboard','name': req_label, 'expected': expected,  'status_code': response.status_code, 'response_time': time.time() - start_time}
    ...
    def search_ticket(self, departure_date, from_station, to_station, expected = True):
        ...
        req_label = 'travelservice'#sys._getframe().f_code.co_name + postfix(expected)
        start_time = time.time()
        with self.client.post(
            ...
            to_log = {'microservice':'travelservice', 'name': req_label, 'expected': expected, 'status_code': response.status_code,
    ...
    def _create_user(self, expected):
        req_label = 'userservice' #'admin_login' + postfix(expected)
        start_time = time.time()
        head = {"Accept": "application/json",
                "Content-Type": "application/json"}
        with self.client.post(url="/api/v1/users/login",
            ...
            to_log = {'microservice':'userservice', 'name': req_label, 'expected': expected, 'status_code': response1.status_code,
                    'response_time': time.time() - start_time}
            self.log_verbose(to_log)                              
        ...
        req_label ='adminuserservice'# sys._getframe().f_code.co_name + postfix(expected)
        start_time = time.time()
        with self.client.post(url = "/api/v1/adminuserservice/users",
            ...
            to_log = {'microservice':'adminuserservice', 'name': req_label, 'expected': expected, 'status_code': response2.status_code,
                    'response_time': time.time() - start_time}
            self.log_verbose(to_log)
    ...
    def _navigate_to_client_login(self, expected = True):
        ...
        with self.client.get('/client_login.html', name = req_label) as response:
            to_log = {'microservice':'uidashboard', 'name': req_label, 'expected': True,  'status_code': response.status_code, 'response_time': time.time() - start_time}
            self.log_verbose(to_log)
    ...
    def login(self, expected):
        ...
        req_label = 'userservice'#sys._getframe().f_code.co_name + postfix(expected)
        start_time = time.time()
        head = {"Accept": "application/json",
                "Content-Type": "application/json"}
        if(expected):
            ...
            to_log = {'microservice':'userservice', 'name': req_label, 'expected': expected, 'status_code': response.status_code,
                    'response_time': time.time() - start_time}
            self.log_verbose(to_log)
        else:
            response = self.client.post(url = "/api/v1/users/login",
    ...
    def get_assurance_types(self, expected):
        head = {"Accept": "application/json",
                "Content-Type": "application/json", "Authorization": self.bearer}
        req_label = 'assuranceservice'#sys._getframe().f_code.co_name + postfix(expected)
        start_time = time.time()
        with self.client.get(
                url = "/api/v1/assuranceservice/assurances/types",
                headers = head,
                name = req_label) as response:
            to_log = {'microservice':'assuranceservice', 'name': req_label, 'expected': expected, 'status_code': response.status_code,
                    'response_time': time.time() - start_time}
            self.log_verbose(to_log)
    ...
    def get_foods(self, expected):
        ...
        head = {"Accept": "application/json",
                "Content-Type": "application/json", "Authorization": self.bearer}
        req_label = 'foodservice'#sys._getframe().f_code.co_name + postfix(expected)
        start_time = time.time()
        with self.client.get(
                url = "/api/v1/foodservice/foods/" + departure_date + "/Shang%20Hai/Su%20Zhou/D1345",
                headers = head,
                name = req_label) as response:
            to_log = {'microservice':'foodservice', 'name': req_label, 'expected': expected, 'status_code': response.status_code,
                    'response_time': time.time() - start_time}
            self.log_verbose(to_log)
    ...
    def select_contact(self, expected):
        head = {"Accept": "application/json",
                "Content-Type": "application/json", "Authorization": self.bearer}
        req_label = 'contactsservice'#sys._getframe().f_code.co_name + postfix(expected)
        start_time = time.time()
        response_contacts = self.client.get(
                url = "/api/v1/contactservice/contacts/account/" + self.user_id,
                headers = head,
                name = req_label)
        to_log = {'microservice':'contactsservice', 'name': req_label, 'expected': expected, 'status_code': response_contacts.status_code,
                'response_time': time.time() - start_time}
        self.log_verbose(to_log)
        ...
        if len(response_as_json_contacts) == 0:
            req_label = 'contactsservice' #'set_new_contact' + postfix(expected)
            start_time = time.time()
    ...
    def finish_booking(self, expected):
        ...
        head = {"Accept": "application/json",
                "Content-Type": "application/json", "Authorization": self.bearer}
        req_label = 'preserveservice'#sys._getframe().f_code.co_name + postfix(expected)
        if(expected):
        ...
        with self.client.post(
                ...
                catch_response = True,
                name = req_label) as response:
            to_log = {'microservice':'preserveservice','name': req_label, 'expected': expected, 'status_code': response.status_code,
                        'response_time': time.time() - start_time}
            self.log_verbose(to_log)
    ...
    def select_order(self, expected):
        head = {"Accept": "application/json",
                "Content-Type": "application/json", "Authorization": self.bearer}
        req_label = 'orderservice'#sys._getframe().f_code.co_name + postfix(expected)
        start_time = time.time()
        ...
        to_log = {'microservice':'orderservice','name': req_label, 'expected': expected, 'status_code': response_order_refresh.status_code,
                'response_time': time.time() - start_time}
        self.log_verbose(to_log)
    ...
    def pay(self, expected):
        head = {"Accept": "application/json",
                "Content-Type": "application/json", "Authorization": self.bearer}
        req_label = 'inside_pay_service'#sys._getframe().f_code.co_name + postfix(expected)
        start_time = time.time()
        if(expected):
            with self.client.post(
                    ...
                    name = req_label) as response:
                to_log = {'microservice':'inside_pay_service','name': req_label, 'expected': expected, 'status_code': response.status_code,
                        'response_time': time.time() - start_time}
                self.log_verbose(to_log)
        else:
            with self.client.post(
    ...
    def cancel_with_no_refund(self, expected):
        head = {"Accept": "application/json",
                "Content-Type": "application/json", "Authorization": self.bearer}
        req_label = 'cancelservice'#sys._getframe().f_code.co_name + postfix(expected)
        start_time = time.time()
        if(expected):
            with self.client.get(
                    ...
                    headers = head,
                    name = req_label) as response:
                to_log = {'microservice':'cancelservice','name': req_label, 'expected': expected, 'status_code': response.status_code,
                        'response_time': time.time() - start_time}
                self.log_verbose(to_log)
    ...
    def get_voucher(self, expected):
        ...
        if(expected):
            with self.client.post(
                    ...
                    name = req_label) as response:
                to_log = {'microservice':'uidashboard','name': req_label, 'expected': expected, 'status_code': response.status_code,
                        'response_time': time.time() - start_time}
                self.log_verbose(to_log)
    ...
    def get_consigns(self, expected):
        req_label = 'consignservice'#sys._getframe().f_code.co_name + postfix(expected)
        start_time = time.time()
        with self.client.get(
                url = "/api/v1/consignservice/consigns/order/" + self.order_id,
                headers = head,
                name = req_label) as response:
            to_log = {'microservice':'consignservice','name': req_label, 'expected': expected, 'status_code': response.status_code,
                    'response_time': time.time() - start_time}
            self.log_verbose(to_log)
        ...
        if(expected):
            response_as_json_consign = self.client.put(
                    ...
                    json = {
                        ...
                        "isWithin": "false"},
                    headers = head)
            to_log = {'microservice':'consignservice','name': req_label, 'expected': expected, 'status_code': response_as_json_consign.status_code,
                    'response_time': time.time() - start_time}
            self.log_verbose(to_log)
        else:
            response_as_json_consign = self.client.put(
